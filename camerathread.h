    ...
    void Verity()
    {
        ...
        if (!m_facenum.isEmpty())
        {
            ...
            if(m_current_people == 1)
            {
                ...
                //存储的地址
                QString dir_str = "D:\\research\\CV\\FaceVeritifity_qml\\facedata\\" + m_facenum + "\\" + QString::number(m_executionCount) + ".jpg";
                //用来判断这个地址的文件夹是否存在
                QString is_dir = "D:\\research\\CV\\FaceVeritifity_qml\\facedata\\" + m_facenum;
                qDebug() << dir_str;
                QDir dir(is_dir);
                if (!dir.exists())
                {
                ...
                if (!faceROI.empty())
                {
                    ...
                    QString csvsave = "D:/research/CV/FaceVeritifity_qml/facedata/" + m_facenum + "/" + QString::number(m_executionCount) + ".jpg" + ";" + m_facenum ;
                    //存放要训练的模型的图片的地址的txt
                    QString csvfilename = "D:\\research\\CV\\FaceVeritifity_qml\\at.txt";
                    //添加数据
